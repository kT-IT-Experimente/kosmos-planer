...
function AuthGate({ onAuthSuccess }) {
    ...
    if (loading) {
        return (
            <div className="min-h-screen flex items-center justify-center p-4">
                <div className="k-panel-glass p-8 max-w-md w-full text-center text-white">
                    <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-[var(--k-accent-teal)] mx-auto mb-4" />
                    <p className="k-body">Initialisiere...</p>
                </div>
            </div>
        );
    }
    ...
    if (authMode === 'verifying') {
        return (
            <div className="min-h-screen flex items-center justify-center p-4">
                <div className="k-panel-glass p-8 max-w-md w-full text-center text-white">
                    <Loader2 className="w-12 h-12 text-[var(--k-accent-teal)] animate-spin mx-auto mb-4" />
                    <h2 className="k-h2 mb-2">Magic Link wird überprüft...</h2>
                    <p className="k-body">Einen Moment bitte.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4">
            <div className="k-panel-glass p-8 max-w-md w-full text-white">
                {/* Header */}
                <div className="text-center mb-8">
                    <h1 className="k-h1 mb-2">Kosmos Planer</h1>
                    <p className="k-body">Festival Program Management</p>
                </div>
                    ...
                    <div className="space-y-4">
                        ...
                        {clientId && (
                            <button onClick={handleGoogleLogin}
                                className="w-full k-btn-secondary py-3 text-sm">
                                <svg className="w-5 h-5 mr-1" viewBox="0 0 24 24"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4" /><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" /><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18A10.96 10.96 0 0 0 1 12c0 1.78.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05" /><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335" /></svg>
                                Mit Google anmelden
                            </button>
                        )}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-white/40 font-medium">ODER</span>
                            <div className="flex-1 h-px bg-white/10" />
                        </div>

                        {/* Email Magic Link */}
                        <button onClick={() => { setAuthMode('email'); setError(null); }}
                            className="w-full k-btn-primary py-3 text-sm">
                            <Mail className="w-5 h-5" />
                            Mit Email-Link anmelden
                        </button>

                        <p className="k-caption text-center mt-2">
                ...
                {authMode === 'email' && (
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <div className="w-6 h-6 bg-[var(--k-accent-teal)] text-[#12141a] rounded-full flex items-center justify-center text-xs font-bold">1</div>
                            <span className="text-sm font-bold text-white">Email-Adresse eingeben</span>
                        </div>

                        <div className="bg-[var(--k-accent-teal)]/10 border border-[var(--k-accent-teal)]/30 rounded-lg p-4 text-sm text-white transition-colors">
                            <p className="font-medium text-[var(--k-accent-teal)] mb-1">So funktioniert's:</p>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-[var(--k-text-muted)] mb-1.5 uppercase tracking-wider">Deine Email-Adresse</label>
                        ...
                        <button onClick={handleRequestMagicLink} disabled={emailSending || !emailInput}
                            className="w-full k-btn-primary py-3 disabled:opacity-50 disabled:cursor-not-allowed text-sm">
                            {emailSending ? (
                                <><Loader2 className="w-4 h-4 animate-spin" /> Wird gesendet...</>
                            ) : (
                                <><Send className="w-4 h-4 mr-1" /> Magic Link anfordern</>
                            )}
                        </button>
                    </div>
                )}
                ...
                {authMode === 'sent' && (
                    <div className="space-y-5 text-center">
                        <div className="w-16 h-16 bg-emerald-900/40 border border-emerald-500/40 rounded-full flex items-center justify-center mx-auto">
                            <CheckCircle2 className="w-8 h-8 text-emerald-400" />
                        </div>

                        <div>
                            <h2 className="text-xl font-bold text-white mb-2">Email verschickt!</h2>
                            <p className="text-sm text-white/60 mb-1">
                                Wir haben einen Magic Link an
                            </p>
                            <p className="font-bold text-[var(--k-accent-teal)] text-base">{emailInput}</p>
                            <p className="text-sm text-white/60 mt-1">gesendet.</p>
                        </div>

                        <div className="bg-black/20 border border-white/10 rounded-lg p-4 text-left">
                            <div className="flex items-center gap-2 mb-2">
                                <div className="w-6 h-6 bg-[var(--k-accent-teal)] text-[#12141a] rounded-full flex items-center justify-center text-xs font-bold">2</div>
                                <span className="text-sm font-bold text-white">Nächster Schritt</span>
                            </div>
                            <ol className="list-decimal list-inside space-y-1.5 text-xs text-white/60">
                                <li>Öffne dein Email-Postfach</li>
                                <li>Suche nach einer Email von <strong className="text-white">Kosmos Planner</strong></li>
                                <li>Klicke auf den Link in der Email</li>
                                <li>Du wirst automatisch eingeloggt</li>
                            </ol>
                            <p className="text-[10px] text-white/40 mt-3">
                                Der Link ist <strong className="text-white/60">7 Tage</strong> gültig. Prüfe auch deinen Spam-Ordner.
                            </p>
                        </div>

                        <div className="flex flex-col gap-2 pt-2">
                            <button onClick={() => { setAuthMode('email'); setEmailSent(false); setError(null); }}
                                className="text-sm text-[var(--k-accent-teal)] hover:text-white font-medium underline transition-colors">
