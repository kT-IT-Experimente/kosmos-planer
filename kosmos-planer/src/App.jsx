...
const Card = React.forwardRef(function Card({ children, className = "", onClick, style, status, ...props }, ref) {
  const statusClass = STATUS_COLORS[status] || 'border-white/10 bg-[var(--k-primary-light)]';
  return (
    <div
      ref={ref}
      onClick={onClick}
...
function SessionCardContent({ session, onClick, onToggleLock, isLocked, hasConflict, conflictTooltip, listeners, attributes, isDimmed, isFavorite, onToggleFavorite, userRole }) {
  ...
  const formatColor = FORMAT_COLORS[session.format] || 'bg-white/10 text-white/90';
  ...
  return (
    ...
    >
      ...
      {activeOverlay === 'notes' && session.notes && (
        <div className="absolute inset-0 bg-[var(--k-primary-light)]/95 z-50 p-3 text-white flex flex-col justify-start items-start text-left backdrop-blur-sm rounded-r animate-in fade-in duration-200 overflow-y-auto custom-scrollbar">
      ...
      <div className="flex justify-between items-start mb-1">
        <div className="flex flex-col overflow-hidden">
          <span className="font-mono text-[10px] font-bold text-white/50 leading-none mb-1">
        ...
        <div className="flex gap-1 shrink-0 z-10 items-center">
          ...
          <button
            ...
            className={`p-1 rounded hover:bg-black/5 transition-colors ${isLocked ? 'text-red-500' : 'text-slate-300 hover:text-white/50'}`}
      ...
      {/* Title */}
      <div className="font-bold text-xs leading-snug mb-1 text-white line-clamp-2" title={isMasked ? 'Reserviert' : session.title}>
      ...
      {!isMasked && <div className="mt-auto space-y-1">
        {session.speakers && (
          <div className="text-[10px] text-white/70 flex flex-wrap items-center gap-1 leading-tight mb-1" title={`Speaker: ${session.speakers}`}>
        ...
        {session.moderators && (
          <div className="text-[10px] text-white/50 flex flex-wrap items-center gap-1 leading-tight" title={`Mod: ${session.moderators}`}>
        ...
        <div className="flex items-center gap-2 text-[9px] text-white/40 pt-1 border-t border-black/5 mt-1">
          <span className="font-mono text-slate-300 text-[8px]">{session.id}</span>
          {session.language && <span className="flex items-center gap-0.5 ml-auto font-bold text-white/50">{session.language.toUpperCase()}</span>}
...
function StageColumn({ stage, children, height }) {
  ...
  return (
    <div ref={setNodeRef} style={{ height: height + HEADER_HEIGHT }} className={`min-w-[280px] w-full max-w-[320px] border-r relative transition-colors ${stage.hidden ? 'border-dashed border-white/20 opacity-50' : 'border-white/10'} ${isOver ? 'bg-[var(--k-accent-teal)]/20' : 'bg-black/10 odd:bg-black/20'}`}>
      <div className={`backdrop-blur border-b p-2 text-center z-20 shadow-sm flex flex-col justify-center ${stage.hidden ? 'bg-[var(--k-primary)]/90 border-dashed border-white/20' : 'bg-[var(--k-primary-light)] border-white/10'}`} style={{ height: HEADER_HEIGHT }}>
        ...
        <div className="flex justify-center gap-3 text-[10px] text-white/50 font-mono mt-0.5">
          {stage.hidden && <span title="Versteckte Bühne (nur Admin)" className="text-white/50">👻</span>}
...
// Normalize legacy Master_Einreichungen status values to canonical format
...
const parsePlannerBatch = (batch, config) => {
  ...
  if (valRanges[4] && valRanges[4].values) {
    ...
    pr = einreichRows.map((r, i) => {
      ...
      return {
        ...
        isPublished: safeString(r[23]).toLowerCase() === 'true' || safeString(r[23]) === '1', // Col X = Veröffentlicht
  ...
  // Columns: A=Bereiche, B=Themen, C=Tags, D=Formate, E=MaxSubmissions, F=LiveMode
  ...
  const configThemen = {
    ...
    liveMode: (() => {
      // Find the first row that has a value in column F (index 5)
      // or check the first row directly (usually where config values sit)
      const v = configRows.find(r => r.length > 5 && safeString(r[5]));
      return v ? (safeString(v[5]).toLowerCase() === 'true' || safeString(v[5]) === '1') : false;
    })(),
  };
...
function App({ authenticatedUser }) {
  ...
  const toggleFavorite = useCallback((session) => {
    setFavorites(prev => {
      ...
      if (config.n8nBaseUrl) {
        ...
        // If a real token exists, use it, otherwise pass anonToken as a placeholder
  ...
  // Sync Live Mode from Backend config when loaded
  useEffect(() => {
  ...
  const toggleLiveMode = useCallback(async () => {
    if (!config.n8nBaseUrl || !authenticatedUser.accessToken) {
      setToast({ msg: 'Konfigurationsfehler: API URL oder Token fehlt', type: 'error' });
      return;
    }

    const nextState = !liveMode;
    setIsTogglingLive(true);

    // Optimistic UI update
    setLiveMode(nextState);

    try {
      const res = await fetch(`${config.n8nBaseUrl}/auth/verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${authenticatedUser.accessToken}` },
        body: JSON.stringify({
      ...
      if (!res.ok || !resData.success) {
        throw new Error(resData.error || 'Toggle Live Mode failed');
      }

      setToast({ msg: `Live Mode ${nextState ? 'aktiviert' : 'deaktiviert'}`, type: 'success' });
    } catch (err) {
      console.error('[App] Failed to toggle live mode:', err);
      // Revert on failure
      setLiveMode(!nextState);
      setToast({ msg: 'Fehler beim Ändern des Live Mode', type: 'error' });
    } finally {
      setIsTogglingLive(false);
    }
  }, [liveMode, config.n8nBaseUrl, authenticatedUser]);
  ...
  const loadData = useCallback(async (options = {}) => {
    ...
    try {
      ...
      ranges.push(`'Master_Einreichungen'!A2:X`);          // index 4: Submissions + Planning (A-O submission, P-W planning, X=isPublished)
      ranges.push(`'Config_Themen'!A2:F`);                 // index 5: Bereiche/Themen/Tags/Formate/MaxSubmissions/LiveMode
      ...
      // Only update if not in the middle of a toggle operation (prevents flicker)
      if (!openCallTogglingRef.current) {
  ...
  const openCallTogglingRef = React.useRef(false);
  ...
  const handleToggleOpenCall = async () => {
    ...
    // Guard: prevent loadData from reverting the toggle while API write is in progress
    openCallTogglingRef.current = true;
    ...
    try {
      ...
      // Release guard after a short delay to let any in-flight loadData complete
      setTimeout(() => { openCallTogglingRef.current = false; }, 2000);
    }
    setTimeout(() => setToast(null), 3000);
  };
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      // --- Solution A: Write planning columns O-X to Master_Einreichungen ---
      // Build rows for columns O-X aligned to rowIndex
      // O=Status, P=Bühne, Q=Startzeit, R=Endzeit, S=Partner, T=Stage_Dispo, U=Tags, V=Moderators, W=Session_ID, X=Veröffentlicht
      ...
      for (let row = 2; row <= maxRow; row++) {
        ...
        if (p) {
          ...
          planningRows.push([
            ...
            p.isPublished ? 'TRUE' : 'FALSE'                   // X: Veröffentlicht
          ]);
        } else {
          planningRows.push(['', '', '', '', '', '', '', '', '', '']); // 10 columns O-X
        }
      }
      ...
      const { ok, error } = await fetchSheets({
        action: 'update',
        spreadsheetId: config.spreadsheetId,
        range: `'Master_Einreichungen'!O2:X`,
        values: planningRows,
      }, token, config.curationApiUrl);
  ...
  const handleExportGraphics = async () => {
    if (!config.n8nBaseUrl) {
      setToast({ msg: '⚠️ API nicht konfiguriert', type: 'error' });
      setTimeout(() => setToast(null), 3000);
      return;
    }

    setToast({ msg: '🖼️ Grafiken werden exportiert...', type: 'info' });
    try {
      const token = authenticatedUser.accessToken || authenticatedUser.magicToken;
      const res = await fetch(`${config.n8nBaseUrl}/export-graphics`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
        body: JSON.stringify({
      ...
      if (res.ok) {
        setToast({ msg: '✅ Export gestartet! Du erhältst eine E-Mail.', type: 'success' });
      } else {
        setToast({ msg: `⚠️ Export Fehler: HTTP ${res.status}`, type: 'error' });
      }
    } catch (e) {
      console.warn('Graphics export failed:', e);
      setToast({ msg: '⚠️ Export konnte nicht gestartet werden.', type: 'error' });
    }
    setTimeout(() => setToast(null), 4000);
  };
  ...
  return (
    <div className="flex flex-col h-screen font-sans overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        {viewMode === 'PLANNER' && (
          <div className="flex flex-col h-full overflow-hidden">
            <header className="bg-[var(--k-primary-light)] border-b border-white/10 px-4 py-2 flex justify-between items-center shrink-0 z-40 shadow-sm">
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setIsSidebarOpen(!isSidebarOpen)}
                  className={`p-2 rounded-xl transition-colors ${isSidebarOpen ? 'bg-[var(--k-accent-teal)]/20 text-[var(--k-accent-teal)]' : 'text-[var(--k-text-muted)] hover:bg-[var(--k-primary-light)]/5 hover:text-white'}`}
                >
                  <Layout className="w-5 h-5" />
                </button>
                <div>
                  <h1 className="text-xl font-black uppercase tracking-widest text-[var(--k-accent-teal)]">
                  ...
                  <div className="flex items-center gap-2 ml-4 flex-1 max-w-md">
                    <div className="relative w-full">
                      ...
                      <input
                        ...
                        className="w-full pl-9 pr-4 py-1.5 bg-white/10 border-none rounded-full text-sm focus:ring-2 focus:ring-indigo-500 transition-all outline-none"
                        value={searchQuery}
                ...
                {!isMobile && (
                  <button
                    ...
                    className="ml-2 k-btn-primary px-3 py-1.5 text-sm"
                  >
                ...
                {['ADMIN', 'CURATION', 'PRODUCTION'].includes(effectiveRole) && (
                  <button
                    onClick={handleExportGraphics}
                    className="ml-2 flex items-center gap-2 px-3 py-1.5 bg-purple-500/20 text-purple-300 hover:bg-purple-500/40 rounded-lg text-sm font-bold transition-colors"
                    title="Grafiken (Session Boards & Bauchbinden) generieren"
                  >
                    <DownloadCloud className="w-4 h-4" />
                    <span>Grafiken Export</span>
                  </button>
                )}
                <button
                  onClick={() => loadData({ manual: true, importProgram: true })}
                  disabled={status.loading}
                  className={`ml-2 flex items-center justify-center p-1.5 rounded transition-colors ${status.loading ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-[var(--k-accent-teal)] hover:bg-[var(--k-accent-teal)]/10'}`}
              ...
              <div className="flex items-center gap-3">
                {curationData.userRole !== 'GUEST' && (
                  <button
                    onClick={handleSync}
                    className={localChanges ? 'k-btn-primary px-3 py-1.5 text-sm animate-pulse flex items-center gap-2' : 'k-btn-secondary px-3 py-1.5 text-sm flex items-center gap-2'}
                  >
                    <UploadCloud className="w-4 h-4 inline" />
                    <span>Speichern</span>
                  </button>
                )}
                <button onClick={() => setShowSettings(!showSettings)} className="p-2 hover:bg-white/10 rounded text-white/50">
                ...
                <div className="relative">
                  ...
                  >
                    {authenticatedUser.picture ? (
                      ...
                      <div className="w-7 h-7 rounded-full bg-[var(--k-accent-teal)] text-[var(--k-primary)] text-white flex items-center justify-center text-xs font-bold">
                  ...
                  {showProfileMenu && (
                    <>
                      ...
                      <div className="absolute right-0 top-full mt-2 w-72 bg-[var(--k-primary-light)] rounded-xl shadow-2xl border border-white/10 z-50 overflow-hidden">
                        <div className="p-4 bg-gradient-to-r from-indigo-50 to-blue-50 border-b border-white/10">
                          <div className="flex items-center gap-3">
                            {authenticatedUser.picture ? (
                              <img src={authenticatedUser.picture} alt="Profile" className="w-12 h-12 rounded-full border-2 border-white shadow-sm" />
                            ) : (
                              <div className="w-12 h-12 rounded-full bg-[var(--k-accent-teal)] text-[var(--k-primary)] text-white flex items-center justify-center text-lg font-bold">
                                {(authenticatedUser.name || authenticatedUser.email || '?').charAt(0).toUpperCase()}
                              </div>
                            )}
                            <div className="flex-1 min-w-0">
                              <p className="font-bold text-white truncate">{authenticatedUser.name || 'User'}</p>
                              <p className="text-xs text-white/50 truncate">{authenticatedUser.email}</p>
            ...
            <DndContext sensors={sensors} onDragStart={handleDragStart} onDragMove={handleDragMove} onDragEnd={handleDragEnd}>
              <div className="flex flex-1 overflow-hidden">
                <div className={`${isSidebarOpen ? 'w-64 border-r border-[#ffffff11]' : 'w-0 border-r-0'} bg-[#121828] text-white flex flex-col shrink-0 z-30 transition-all duration-300 overflow-hidden`}>
                  <div className="p-4 border-b border-[#ffffff11] bg-[#ffffff05]">
                    <h3 className="k-caption flex items-center gap-2"><PieChart className="w-4 h-4 inline" /> Analyse</h3>
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div className="bg-[var(--k-primary-light)] p-2 rounded border text-center text-xs font-bold">
                        <div className="text-[var(--k-accent-teal)]">{analysis.genderCounts.w}</div>
                        <div className="text-[8px] text-white/40 uppercase">Frauen (W)</div>
                      </div>
                      <div className="bg-[var(--k-primary-light)] p-2 rounded border text-center text-xs font-bold">
                        <div className="text-[var(--k-accent-teal)]">{analysis.genderCounts.m}</div>
                        <div className="text-[8px] text-white/40 uppercase">Männer (M)</div>
                      </div>
                      <div className="bg-[var(--k-primary-light)] p-2 rounded border text-center text-xs font-bold col-span-2">
                        <div className="flex justify-between items-center px-1">
                          <span className="text-[8px] text-white/40 uppercase">Partner-Anteil:</span>
                          <span className="text-[var(--k-accent-teal)]">{analysis.partnerPercent}%</span>
                        </div>
                        <div className="w-full bg-white/10 h-1 mt-1 rounded-full overflow-hidden">
                          <div className="bg-[var(--k-accent-teal)]/100 h-full" style={{ width: `${analysis.partnerPercent}%` }}></div>
                        </div>
                      </div>
                      <div className="bg-[var(--k-primary-light)] p-2 rounded border text-center text-xs font-bold col-span-2">
                        <div className="flex justify-between items-center px-1">
                          <span className="text-[8px] text-white/40 uppercase">Bühnen-Belegung:</span>
                          <span className="text-[var(--k-accent-teal)]">{analysis.occupancyPercent}%</span>
                        </div>
                        <div className="w-full bg-white/10 h-1 mt-1 rounded-full overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto p-2">
                    <div className="text-[10px] font-bold text-white/40 uppercase p-2 tracking-widest">SprecherInnen ({data.speakers.length})</div>
                    {data.speakers.slice(0, 50).map(s => <div key={s.id} className="text-[10px] py-1 px-2 hover:bg-white/10 rounded truncate">{s.fullName}</div>)}
                  </div>
                </div>

                <div className="flex-1 flex flex-col overflow-hidden relative">
                  {/* INBOX */}
                  <div className={`k-panel-glass border-b border-white/10 flex flex-col transition-all duration-300 ease-in-out ${isInboxCollapsed ? 'h-10' : 'h-48'}`}>
                    <div className="flex items-center justify-between px-3 py-1.5 border-b border-white/10 bg-black/40 shrink-0">
                      <div className="flex items-center gap-3">
                        <h3 className="k-caption flex items-center gap-2">
                          <PlusCircle className="w-3.5 h-3.5 text-[var(--k-accent-teal)]" />
                          Inbox ({filteredAndSortedInbox.length})
                        </h3>
                        {!isInboxCollapsed && (
                          <div className="flex items-center gap-2 border-l border-white/20 pl-3 ml-1">
                            <span className="text-[9px] text-white/40 font-bold uppercase">Sort:</span>
                            <select
                              className="text-[9px] bg-transparent border-none focus:ring-0 font-bold text-slate-300 cursor-pointer outline-none"
                              value={inboxSortBy}
                      ...
                      <button
                        ...
                        className="p-1 hover:bg-[var(--k-primary-light)]/10 rounded transition-colors text-white/40 hover:text-white"
                        ...
                        <SortableContext id={INBOX_ID} items={filteredAndSortedInbox.map(p => p.id)}>
                          <DroppableStage id={INBOX_ID} className="flex gap-2 min-h-full items-start">
                            ...
                            {filteredAndSortedInbox.length === 0 && (
                              <div className="flex flex-col items-center justify-center w-full h-full text-white/40 animate-pulse">
                  ...
                  <div className="flex-1 overflow-auto relative custom-scrollbar flex">
                    <div className="w-10 bg-[var(--k-primary-light)] border-r border-white/10 shrink-0 sticky left-0 z-30 shadow-sm" style={{ minHeight: timelineHeight + HEADER_HEIGHT }}>
                      {Array.from({ length: config.endHour - config.startHour }).map((_, i) => {
                        const hour = config.startHour + i;
                        return (
                          <div
                            key={hour}
                            className="absolute w-full border-t border-white/10 text-[10px] font-bold text-white/50 pl-1 pt-0.5"
              ...
              <DragOverlay>
                {activeDragItem ? (
                  <Card status={activeDragItem.status} className="w-[200px] bg-[var(--k-accent-teal)] text-[var(--k-primary)] text-white shadow-2xl">
        ...
        {viewMode === 'CURATION' && (
          <div className="flex flex-col h-full overflow-hidden">
            <header className="bg-[var(--k-primary-light)] text-white px-4 py-2 flex justify-between items-center shadow-lg shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 text-xs px-3 py-1 rounded-full transition-all font-bold uppercase tracking-wider border ${liveMode
                    ...
                    : 'bg-[var(--k-primary-light)] border-[var(--k-accent-teal)]/30 text-indigo-200 hover:bg-indigo-700'
                    } ${isTogglingLive ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  {isTogglingLive ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    <span className={`w-2 h-2 rounded-full ${liveMode ? 'bg-[var(--k-primary-light)]' : 'bg-indigo-400'}`} />
                  )}
                  {liveMode ? '🔴 LIVE' : 'Live Mode'}
                </button>
                <button onClick={() => setViewMode('PLANNER')} className="text-xs bg-[var(--k-primary-light)] hover:bg-indigo-700 px-3 py-1 rounded transition-colors uppercase font-bold tracking-widest">Planner View</button>
              </div>
            </header>
            <AdminDashboard
              users={curationData.users || []}
            ...
            <div className="flex-1 overflow-auto p-4">
              <div className="max-w-5xl mx-auto">
                {(data.organisations || []).length === 0 ? (
                  <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 p-6 text-center text-white/40">
                    <p className="text-sm">Noch keine Organisationen in Config_Organisations hinterlegt.</p>
                  </div>
                ) : (
                  <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 overflow-hidden">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-white/5 border-b border-white/10">
                          <th className="text-left py-3 px-4 text-xs font-bold text-white/50 uppercase">Organisation</th>
                          <th className="text-left py-3 px-4 text-xs font-bold text-white/50 uppercase">E-Mail</th>
                          <th className="text-left py-3 px-4 text-xs font-bold text-white/50 uppercase">Status</th>
                          <th className="text-left py-3 px-4 text-xs font-bold text-white/50 uppercase">Sessions</th>
                          <th className="text-left py-3 px-4 text-xs font-bold text-white/50 uppercase">Links</th>
                        </tr>
                      </thead>
                      <tbody>
                        {(data.organisations || []).map((org, i) => {
                          ...
                          return (
                            <React.Fragment key={org.email || i}>
                              <tr className={`border-b border-white/5 hover:bg-white/5/50 cursor-pointer ${isExpanded ? 'bg-blue-50/30' : ''}`} onClick={() => setExpandedOrgEmail(isExpanded ? null : org.email)}>
                                <td className="py-3 px-4">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      ...
                                      {org.beschreibung && <div className="text-[10px] text-white/40 truncate max-w-[200px]">{org.beschreibung}</div>}
                                    </div>
                                  </div>
                                </td>
                                <td className="py-3 px-4 text-white/70 text-xs">{org.email}</td>
                                ...
                                <td className="py-3 px-4">
                                  <span className={`text-xs font-bold ${orgSessions.length > 0 ? 'text-[var(--k-accent-teal)]' : 'text-white/40'}`}>
                                      ...
                                      {orgSessions.map((session, si) => {
                                        ...
                                        return (
                                          <div key={session.id || si}
                                            onClick={() => { setEditingSession(session); setIsModalOpen(true); }}
                                            className={`border rounded-lg p-3 cursor-pointer hover:shadow-md transition-shadow ${isFixed ? 'border-green-200 bg-green-50/50' : isAccepted ? 'border-blue-200 bg-blue-50/50' : 'border-white/10 bg-[var(--k-primary-light)]'}`}
                                          >
                                            <div className="flex items-center justify-between gap-2">
                                              <h4 className="text-sm font-bold text-white">{session.title || 'Ohne Titel'}</h4>
                                            ...
                                            <div className="flex items-center gap-3 mt-1 text-[10px] text-white/50">
                                              {stageName && <span className="bg-white/10 px-1.5 py-0.5 rounded font-bold">{stageName}</span>}
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    {filterTabs.map(tab => (
                      <button key={tab.key} onClick={() => { setSpeakerDashFilter(tab.key); setExpandedSpeakerId(null); }}
                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition-colors ${speakerDashFilter === tab.key ? 'bg-purple-600 text-white' : 'bg-white/10 text-white/70 hover:bg-white/20'}`}>
                        {tab.label} ({tab.count})
                      </button>
                    ))}
                  </div>
                    ...
                    <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 overflow-hidden">
                      <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="bg-white/5 border-b border-white/10">
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Name</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Status</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Briefing</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Sessions</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Profil</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Reise</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Honorar</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Catering</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Vertrag</th>
                              <th className="text-left py-2 px-3 text-[10px] font-bold text-white/50 uppercase">Ehrenamt</th>
                            </tr>
                          </thead>
                          <tbody>
                            {filtered.map((spk, i) => {
                              ...
                              return (
                                <React.Fragment key={spk.id || i}>
                                  <tr className={`border-b border-white/5 hover:bg-white/5/50 cursor-pointer ${isExpanded ? 'bg-purple-50/30' : ''}`}
                                    onClick={() => setExpandedSpeakerId(isExpanded ? null : spk.id)}>
                                    {/* Name */}
                                    <td className="py-2 px-3">
                                      <div className="font-bold text-white text-xs">{spk.fullName || '—'}</div>
                                      <div className="text-[10px] text-white/40">{spk.email}</div>
                                    ...
                                    <td className="py-2 px-3">
                                      {briefingItems.length > 0 ? (
                                        <div className="flex flex-wrap gap-0.5 max-w-[140px]">
                                          <span className="text-[8px] text-white/40 font-bold mr-1">{briefingItems.length}×</span>
                                          {briefingItems.map((b, bi) => (
                                            <span key={bi} className="px-1 py-0 rounded bg-[var(--k-accent-teal)]/10 text-indigo-700 text-[8px] font-bold">{b}</span>
                                          ))}
                                        </div>
                                    ...
                                    {/* Sessions */}
                                    <td className="py-2 px-3">
                                      <span className={`text-xs font-bold ${spkSessions.length > 0 ? 'text-purple-600' : 'text-white/40'}`}>
                                    ...
                                    <td className="py-2 px-3" onClick={e => e.stopPropagation()}>
                                      {isAdmin ? (
                                        <div className="space-y-1">
                                          ...
                                          {nights !== null && <div className="text-[8px] text-white/50">{nights} Nächte</div>}
                                        </div>
                                      ) : (
                                    ...
                                    <td className="py-2 px-3" onClick={e => e.stopPropagation()}>
                                      {isAdmin ? (
                                        ...
                                        <span className="text-[10px] text-white/50">{spk.honorar || '—'}</span>
                                      )}
                                    </td>
                                    ...
                                    <td className="py-2 px-3" onClick={e => e.stopPropagation()}>
                                      {isAdmin ? (
                                        ...
                                        <span className="text-[9px] text-white/50">{spk.catering || '—'}</span>
                                      )}
                                    </td>
                                    {/* Vertrag (AE) */}
                                    <td className="py-2 px-3" onClick={e => e.stopPropagation()}>
                                    ...
                                    <td className="py-2 px-3" onClick={e => e.stopPropagation()}>
                                      {isAdmin ? (
                                        <button onClick={() => updateSpeakerField(spk, 'AG', spk.ehrenamtsverguetung ? 'FALSE' : 'TRUE')}
                                          className={`px-2 py-0.5 rounded text-[10px] font-bold ${spk.ehrenamtsverguetung ? 'bg-green-100 text-green-700' : 'bg-white/10 text-white/50'}`}>
                                          {spk.ehrenamtsverguetung ? '✓ Ja' : '—'}
                                        </button>
                                      ) : (
                                        <span className="text-[10px] text-white/50">{spk.ehrenamtsverguetung ? 'Ja' : '—'}</span>
                                      )}
                                    </td>
                                  </tr>
                                  {isExpanded && (
                                    <tr>
                                      <td colSpan="10" className="bg-white/5 px-3 py-3">
                                        <div className="grid grid-cols-2 gap-4 mb-3 text-[10px] text-white/50">
                                        ...
                                        {spkSessions.length > 0 ? (
                                          <div className="space-y-2">
                                            <p className="text-[10px] font-bold text-white/50 uppercase">Sessions ({spkSessions.length})</p>
                                            {spkSessions.map((session, si) => {
                                              const stageName = data.stages.find(s => s.id === session.stage)?.name || '';
                                              return (
                                                <div key={session.id || si}
                                                  onClick={() => { setEditingSession(session); setIsModalOpen(true); }}
                                                  className="border rounded-lg p-2 cursor-pointer hover:shadow-md transition-shadow bg-[var(--k-primary-light)] border-white/10">
                                                  <div className="flex items-center justify-between gap-2">
                                                    <span className="text-xs font-bold text-white">{session.title || 'Ohne Titel'}</span>
                                                    <span className="px-1.5 py-0.5 rounded text-[9px] font-bold uppercase bg-white/10 text-white/70">{session.status || 'Vorschlag'}</span>
                                                  </div>
                                                  <div className="flex items-center gap-2 mt-1 text-[10px] text-white/50">
                                                    {stageName && <span className="bg-white/10 px-1 py-0.5 rounded font-bold">{stageName}</span>}
                                          ...
                                          <p className="text-[10px] text-white/40">Keine Sessions verknüpft.</p>
                                        )}
                                      </td>
                                    </tr>
                                  )}
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6">
            {hasRole('SPEAKER') && !hasRole('ADMIN', 'TEILNEHMENDE') ? (
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                ...
                {mySessions.length > 0 ? (
                  <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 p-6">
                    <h2 className="text-lg font-bold text-white mb-4">📋 Meine Sessions ({mySessions.length})</h2>
                    <div className="space-y-3">
                      {mySessions.map((session, i) => {
                        ...
                        return (
                          <div key={session.id || i} className={`border rounded-lg p-4 ${isFixed ? 'border-green-200 bg-green-50/30' : isAccepted ? 'border-blue-200 bg-blue-50/30' : 'border-white/10'}`}>
                            ...
                            <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-white/50">
                              {showSchedule && stageName && <span className="bg-white/10 text-white/70 px-1.5 py-0.5 rounded font-bold">{stageName}</span>}
              ...
              <div className="max-w-3xl mx-auto space-y-6">
                ...
                {myOrgRecord && (
                  <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 p-6">
                    <div className="flex items-center gap-4 mb-4">
                      {myOrgRecord.logoUrl && (
                        <img src={myOrgRecord.logoUrl} alt={myOrgRecord.name} className="w-16 h-16 rounded-lg object-cover border border-white/10" />
                      )}
                      <div>
                        <h2 className="text-lg font-bold text-white">{myOrgRecord.name || 'Meine Organisation'}</h2>
                      ...
                      <button onClick={() => setViewMode('ORG_PROFILE')} className="ml-auto px-3 py-1.5 bg-[var(--k-accent-teal)]/10 text-[var(--k-accent-teal)] rounded-lg text-xs font-bold hover:bg-indigo-100 transition-colors">
                        Profil bearbeiten
                      </button>
                    </div>
                  </div>
                ...
                {myOrgSessions.length > 0 ? (
                  <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 p-6">
                    <h2 className="text-lg font-bold text-white mb-4">🏢 Partner-Sessions ({myOrgSessions.length})</h2>
                    <div className="space-y-3">
                      {myOrgSessions.map((session, i) => {
                        ...
                        return (
                          <div key={session.id || i} className={`border rounded-lg p-4 ${isFixed ? 'border-green-200 bg-green-50/30' : isAccepted ? 'border-blue-200 bg-blue-50/30' : 'border-white/10'}`}>
                            ...
                            {isVorschlag && (
                              <p className="mt-2 text-[10px] text-white/40 italic">Bühne und Zeiten werden nach der Kuration zugewiesen.</p>
                            )}
                          </div>
                        );
                      })}
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="k-panel-glass p-6 bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10">
                  <h2 className="text-lg font-bold text-white mb-4">🏢 Organisations-Profil</h2>
                  <form onSubmit={async (e) => {
                    ...
                    <div>
                      <label className="text-xs font-bold text-white/50 uppercase block mb-1">Name der Organisation</label>
                      <input name="orgName" defaultValue={myOrgRecord?.name || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" required />
                    </div>
                    <div>
                      <label className="text-xs font-bold text-white/50 uppercase block mb-1">Beschreibung</label>
                      <textarea name="orgBeschreibung" defaultValue={myOrgRecord?.beschreibung || ''} rows={4} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" />
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="text-xs font-bold text-white/50 uppercase block mb-1">Webseite</label>
                        <input name="orgWebseite" defaultValue={myOrgRecord?.webseite || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" placeholder="https://..." />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-white/50 uppercase block mb-1">Logo URL</label>
                        <input name="orgLogoUrl" defaultValue={myOrgRecord?.logoUrl || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" placeholder="https://..." />
                        {myOrgRecord?.logoUrl && (
                          <div className="mt-2 flex items-center gap-3">
                            <img src={myOrgRecord.logoUrl} alt="Logo" className="w-16 h-16 rounded-lg object-cover border border-white/10" />
                            <span className="text-[10px] text-white/40">Aktuelle Vorschau</span>
                          </div>
                        )}
                      </div>
                    </div>
                    <div className="grid grid-cols-3 gap-4">
                      <div>
                        <label className="text-xs font-bold text-white/50 uppercase block mb-1">Instagram</label>
                        <input name="orgInstagram" defaultValue={myOrgRecord?.instagram || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" placeholder="@handle" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-white/50 uppercase block mb-1">LinkedIn</label>
                        <input name="orgLinkedin" defaultValue={myOrgRecord?.linkedin || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" />
                      </div>
                      <div>
                        <label className="text-xs font-bold text-white/50 uppercase block mb-1">Sonstiges</label>
                        <input name="orgSocialSonstiges" defaultValue={myOrgRecord?.socialSonstiges || ''} className="w-full border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-100" />
                      </div>
                    </div>
                    <div className="flex gap-2 pt-2">
                      <button type="submit" className="px-4 py-2 bg-[var(--k-accent-teal)] text-[var(--k-primary)] text-white rounded-lg text-sm font-bold hover:bg-indigo-700 transition-colors">💾 Speichern</button>
                      <button type="button" onClick={() => setViewMode('ORG_SESSIONS')} className="px-4 py-2 bg-white/10 text-white/70 rounded-lg text-sm font-bold hover:bg-white/20 transition-colors">Abbrechen</button>
                    </div>
                  </form>
                </div>

                {/* Linked Sessions below profile form */}
                <div className="k-panel-glass bg-[var(--k-primary-light)] rounded-xl shadow-sm border border-white/10 p-6 mt-6">
                  <h2 className="text-lg font-bold text-white mb-4">📄 Verknüpfte Sessions ({myOrgSessions.length})</h2>
                  {myOrgSessions.length > 0 ? (
                    <div className="space-y-3">
                      {myOrgSessions.map((session, i) => {
                        const isPending = (session.partner || '').startsWith('pending:');
                        ...
                        const stageName = data.stages.find(s => s.id === session.stage)?.name || '';
                        return (
                          <div key={session.id || i} className={`border rounded-lg p-4 ${isPending ? 'border-amber-200 bg-amber-50/30' : isFixed ? 'border-green-200 bg-green-50/30' : isAccepted ? 'border-blue-200 bg-blue-50/30' : 'border-white/10'}`}>
                            <div className="flex items-start justify-between gap-2">
                              <h3 className="font-bold text-sm text-white">{session.title || 'Ohne Titel'}</h3>
                              <div className="flex items-center gap-2 shrink-0">
                                ...
                                <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase ${isFixed ? 'bg-green-100 text-green-700' : isAccepted ? 'bg-blue-100 text-blue-700' : 'bg-white/10 text-white/70'}`}>
                                  {session.status || 'Vorschlag'}
                                </span>
                              </div>
                            </div>
                            ...
                            {isPending && (
                              <div className="flex gap-2 mt-3">
                                ...
                                <button
                                  ...
                                  className="px-3 py-1.5 bg-white/20 text-white/70 rounded-lg text-xs font-bold hover:bg-slate-300 transition-colors"
                                >
                                  ✗ Ablehnen
                                </button>
                              </div>
                    ...
                    <p className="text-sm text-white/40 text-center">Noch keine Sessions verknüpft.</p>
                  )}
                </div>
              </div>
            ) : (
              <SessionSubmission
                ...
                openCallClosed={openCallClosed && !hasRole('ADMIN', 'CURATOR', 'REVIEWER')}
                onSuccess={(newTitle) => {
        ...
        {viewMode === 'REGISTER' && (
          <div className="flex-1 overflow-y-auto bg-white/5 p-6 space-y-6">
            <SpeakerRegistration
            ...
            <div className="text-center">
              <button
                ...
                className="text-sm text-[var(--k-accent-teal)] hover:text-indigo-800 underline"
              >
      ...
      <div className="h-10 bg-[var(--k-primary)] flex items-center justify-center gap-4 sm:gap-8 shrink-0 border-t border-slate-800 overflow-x-auto">
        {/* Planer: ADMIN, CURATOR, REVIEWER, PRODUCTION */}
        {hasRole('ADMIN', 'CURATOR', 'REVIEWER', 'PRODUCTION') && (
          <button onClick={() => setViewMode('PLANNER')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'PLANNER' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
        ...
        {hasRole('ADMIN', 'TEILNEHMENDE', 'SPEAKER') && (
          <button onClick={() => setViewMode('SUBMIT')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'SUBMIT' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
        ...
        {hasRole('ADMIN', 'CURATOR') && (
          <button onClick={() => setViewMode('CURATION')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'CURATION' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
        ...
        {hasRole('ADMIN', 'REVIEWER') && (
          <button onClick={() => setViewMode('ORG_DASHBOARD')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'ORG_DASHBOARD' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
            🏢 Organisationen
          </button>
        )}
        ...
        {hasRole('ADMIN', 'REVIEWER') && (
          <button onClick={() => setViewMode('SPRECHERIN_DASHBOARD')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'SPRECHERIN_DASHBOARD' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
            🎤 SprecherInnen
          </button>
        )}
        ...
        {hasRole('ADMIN', 'SPEAKER', 'TEILNEHMENDE') && (
          <button onClick={() => setViewMode('PROFILE')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'PROFILE' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
        ...
        {hasRole('ORGANISATION') && (
          <button onClick={() => setViewMode('ORG_SESSIONS')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'ORG_SESSIONS' || viewMode === 'ORG_PROFILE' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
            🏢 Organisation
          </button>
        )}
        ...
        {hasRole('ADMIN', 'REVIEWER') && (
          <button onClick={() => setViewMode('ADMIN')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'ADMIN' ? 'text-[var(--k-accent-teal)]' : 'text-white/50 hover:text-white'}`}>
        ...
        {hasRole('ADMIN', 'PRODUCTION') && (
          <button onClick={() => setViewMode('PRODUCTION')} className={`flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap ${viewMode === 'PRODUCTION' ? 'text-orange-400' : 'text-white/50 hover:text-white'}`}>
            🎛️ Produktion
          </button>
        )}

        {/* Logout */}
        <button onClick={handleLogout} className="flex items-center gap-1.5 text-[10px] font-bold uppercase transition-all whitespace-nowrap text-white/50 hover:text-red-400" title="Abmelden">
      ...
      {/* Settings Modal */}
      {
        showSettings && (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div className="bg-[var(--k-primary-light)] p-6 rounded-xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <h3 className="text-xs font-bold uppercase text-white/50">Sheet Config</h3>
                  ...
                  <input
                    ...
                    className="w-full border rounded p-1.5 text-xs text-white focus:outline-none focus:ring-1 focus:ring-indigo-500"
                  />
                ...
                <div className="space-y-2 border-t pt-2">
                  <h3 className="text-xs font-bold uppercase text-white/50">Auth</h3>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {config.googleClientId !== localStorage.getItem('kosmos_server_client_id') && (
                      <button
                        ...
                        className="px-2 py-2 bg-white/10 border rounded text-[10px] hover:bg-white/20"
                  ...
                  {config.googleClientId !== localStorage.getItem('kosmos_server_client_id') && (
                    <div className="text-[10px] text-white/50 mt-1 bg-white/5 p-2 rounded">
                  ...
                  <div className="flex items-center gap-2 pt-4 border-t mt-4">
                    <button
                      ...
                      className="text-[10px] bg-[var(--k-primary-light)] text-white px-3 py-2 rounded hover:bg-slate-700 flex items-center gap-2 transition-all font-bold uppercase tracking-wider shadow-sm"
                    >
                      <LayoutDashboard className="w-3.5 h-3.5" /> Mock-Daten laden (Test)
                    </button>
                    <p className="text-[9px] text-white/40 italic flex-1">Fügt temporäre Sessions hinzu, um das Dashboard zu testen.</p>
                  </div>
                </div>
              </div>
