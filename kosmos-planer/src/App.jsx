...
const parsePlannerBatch = (batch, config) => {
  ...
  // --- Parse Master_Einreichungen (valRanges[5]) ---
  ...
  if (valRanges[5] && valRanges[5].values) {
    const einreichRows = valRanges[5].values.filter(r => safeString(r[4])); // Must have a title (col E)
  ...
  if (import.meta.env.DEV) {
    console.log(`[parsePlannerBatch] Final speaker count: ${sp.length} (from sheet: ${(valRanges[0].values || []).length} rows, submissions: ${valRanges[5]?.values?.length || 0} rows)`);
...
function App({ authenticatedUser }) {
  ...
  const loadData = useCallback(async (options = {}) => {
    ...
    try {
      ...
      ranges.push(`'Master_Einreichungen'!A2:X`);          // index 4: Submissions + Planning (A-O submission, P-V planning, W=Session_ID, X=Session_ID_2)
      ranges.push(`'Config_Themen'!A2:G`);                 // index 5: Bereiche/Themen/Tags/Formate/MaxSubmissions/OpenCallStatus
      ranges.push(`'Master_Ratings'!A2:F`);                 // index 6: Ratings
      ranges.push(`'Config_Users'!A2:C`);                   // index 7: Users (email, role, name)
      ranges.push(`'Config_Organisations'!A2:I`);           // index 8: Organisations (incl status col I)
      ...
      // Read Open Call status from Config_Themen G2 (index 5, col 6)
      // The array configRows comes from valRanges[5]
      const configThemenValues = batch.valueRanges?.[5]?.values || [];
      const openCallValue = safeString(configThemenValues[0]?.[6]).toUpperCase();
      if (openCallValue && !openCallTogglingRef.current) {
        setOpenCallClosed(openCallValue === 'CLOSED');
      }
      setData(prev => {
    ...
    <div className="flex flex-col h-screen bg-transparent font-sans overflow-hidden text-[var(--k-text-main)]">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden relative">
        {viewMode === 'PLANNER' && (
          <div className="flex flex-col h-full overflow-hidden">
              ...
              <div className="flex items-center gap-4">
                ...
                {!isMobile && (
                  <button
                    ...
                    className="ml-2 flex items-center gap-2 px-3 py-1.5 bg-[#81C7A9] text-[#161616] hover:bg-indigo-700 text-white rounded text-sm font-bold shadow-sm transition-all hover:scale-105 active:scale-95"
                  >
                ...
                <div className="relative">
                  ...
                  >
                    {authenticatedUser.picture ? (
                      ...
                      <div className="w-7 h-7 rounded-full bg-[#81C7A9] text-[#161616] text-white flex items-center justify-center text-xs font-bold">
                        ...
                        <div className="p-4 bg-gradient-to-r from-indigo-50 to-blue-50 border-b border-[#161616]/20">
                          <div className="flex items-center gap-3">
                            {authenticatedUser.picture ? (
                              ...
                              <div className="w-12 h-12 rounded-full bg-[#81C7A9] text-[#161616] text-white flex items-center justify-center text-lg font-bold">
                                {(authenticatedUser.name || authenticatedUser.email || '?').charAt(0).toUpperCase()}
                              </div>
                            )}
            ...
            <DndContext sensors={sensors} onDragStart={handleDragStart} onDragMove={handleDragMove} onDragEnd={handleDragEnd}>
              ...
              <DragOverlay>
                {activeDragItem ? (
                  <Card status={activeDragItem.status} className="w-[200px] bg-[#81C7A9] text-[#161616] text-white shadow-2xl">
        ...
        {viewMode === 'ADMIN' && hasRole('ADMIN', 'REVIEWER') && (
          <div className="flex flex-col h-full overflow-hidden">
            <header className="bg-[#351E8B] text-white px-4 py-2 flex justify-between items-center shadow-lg shrink-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`flex items-center gap-2 text-xs px-3 py-1 rounded-full transition-all font-bold uppercase tracking-wider border ${liveMode
                    ...
                    : 'bg-[#4A2EBA] border-[#351E8B] text-indigo-200 hover:bg-indigo-700'
                    }`}
                >
                  <span className={`w-2 h-2 rounded-full ${liveMode ? 'bg-white' : 'bg-indigo-400'}`} />
                  {liveMode ? '🔴 LIVE' : 'Live Mode'}
                </button>
                <button onClick={() => setViewMode('PLANNER')} className="text-xs bg-[#4A2EBA] hover:bg-indigo-700 px-3 py-1 rounded transition-colors uppercase font-bold tracking-widest">Planner View</button>
              </div>
            </header>
            <AdminDashboard
              users={curationData.users || []}
          ...
          <div className="flex-1 overflow-y-auto bg-transparent p-6 space-y-6">
            ...
            <div className="text-center">
              <button
                ...
                className="text-sm text-[#351E8B] hover:text-indigo-800 underline"
              >
